...
=Shift+Enter= evaluates the expression before it and display the result. =Ctrl/Cmd+Enter= compiles the current top-level form (i.e. surrounding or before the cursor). This is particularly useful for defining functions one by one. =Ctrl/Cmd+Shift+Enter= compiles the current top-level form with high debug settings.
